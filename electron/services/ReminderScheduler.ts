...
export type ReminderEvent = { kind: "hydration" | "break"; title: string; body: string };
...
export class ReminderScheduler {
  ...
  snooze(kind: ReminderEvent["kind"], minutes: number): void {
    const until = Date.now() + Math.max(1, Math.min(1_440, Math.round(minutes))) * 60_000;
    if (kind === "hydration") this.hydrationDueAt = until;
    else this.breakUntil = until;
  }
  ...
  tick(snapshot: ActivitySnapshot, settings: Settings, interruptionsAllowed: boolean): ReminderEvent[] {
    ...
    if (now >= this.hydrationDueAt) {
      this.hydrationDueAt = now + hydrationMinutes * 60_000;
      events.push({ kind: "hydration", title: "喝水提醒", body: "起来喝口水、活动一下吧。" });
    }
    const productive = productiveActivityKinds.has(snapshot.activityKind);
    if (now >= this.breakUntil && snapshot.presenceState === "active" && productive) {
      this.productiveSeconds += elapsed;
    }
    if (this.productiveSeconds >= Math.max(1, settings.reminders.focusMinutes) * 60) {
      this.productiveSeconds = 0;
      this.breakUntil = now + Math.max(1, settings.reminders.breakMinutes) * 60_000;
      events.push({ kind: "break", title: "休息提醒", body: `已经专注一段时间，休息 ${Math.max(1, settings.reminders.breakMinutes)} 分钟吧。` });
    }
    return events;
  }
}
